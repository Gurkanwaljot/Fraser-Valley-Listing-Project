...
import { useCreateRealtor } from '../../hooks/useRealtors';
...
export default function RealtorCreatePage() {
  ...
  const validateEmail = async () => {
    ...
    const isUnique = await checkRealtorEmailUniqueness(form.email, user!.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createRealtor.mutateAsync({
      ...
      showToast('Realtor added successfully');
      navigate(ROUTES.DASHBOARD_REALTORS);
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'Failed to create realtor', 'error');
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
                ...
                error={!!errors.email}
                helperText={errors.email || (emailChecking ? 'Checking...' : '')}
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
          ...
          <Stack direction="row" spacing={2} sx={{ mt: 4 }}>
            <Button
              type="submit"
              variant="contained"
              disabled={createRealtor.isPending}
            >
              {createRealtor.isPending ? 'Saving...' : 'Add Realtor'}
            </Button>
            <Button
              variant="outlined"
              onClick={() => navigate(ROUTES.DASHBOARD_REALTORS)}
