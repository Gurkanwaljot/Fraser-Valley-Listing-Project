...
export default function TemplateD({ listing, photos, realtor, status, imageMap, theme }: PostTemplateProps) {
  ...
  const gridCount = extras.length >= 3 ? extras.length : 0;

  const photoH = photoZoneHeight(photos.length, 620, 880);
  ...
  const heroH = gridCount === 0 ? photoH : Math.round(photoH * 0.66);
  ...
  return (
    ...
    >
      <Box sx={{ position: 'relative', width: '100%', height: heroH, overflow: 'hidden' }}>
        ...
        <ScrimPanel scrim={theme.scrim} from={32} strength={0.92} />
        <Box sx={{ position: 'absolute', left: 60, right: 60, bottom: 48, zIndex: 2 }}>
          <Eyebrow color={theme.accent}>Private Collection</Eyebrow>
          <Box sx={{ mt: '16px' }}>
            <AutoFitText
              maxFontSize={92}
              minFontSize={48}
