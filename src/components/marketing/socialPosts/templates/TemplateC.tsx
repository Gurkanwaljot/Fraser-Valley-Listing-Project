...
export default function TemplateC({ listing, photos, realtor, status, imageMap, theme }: PostTemplateProps) {
  ...
  const photoH = photoZoneHeight(photos.length, 560, 820);
  ...
  const heroH = gridCount === 0 ? photoH : Math.round(photoH * 0.58);
  const gridH = gridCount === 0 ? 0 : photoH - heroH - 6;
  const rowH = gridRows === 0 ? 0 : Math.floor((gridH - (gridRows - 1) * 6) / gridRows);
  ...
  return (
    ...
    >
      <Box sx={{ position: 'relative', width: '100%', height: heroH, overflow: 'hidden' }}>
        ...
        <ScrimPanel scrim={theme.scrim} from={38} strength={0.86} />
        <Box sx={{ position: 'absolute', left: 56, right: 56, bottom: 40, zIndex: 2 }}>
          <Eyebrow color={theme.accent}>The Collection</Eyebrow>
          <Box sx={{ mt: '14px' }}>
            <AutoFitText
              maxFontSize={76}
              minFontSize={40}
      ...
      <FitScale height={contentH} align="center">
        <Box sx={{ padding: '0 56px', display: 'flex', flexDirection: 'column', gap: '22px' }}>
